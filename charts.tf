resource "github_repository" "charts" {
  name                 = "charts"
  description          = "The ContainerSSH Helm charts"
  has_issues           = true
  has_projects         = false
  has_wiki             = false
  has_downloads        = false
...
resource "github_team_repository" "charts" {
...
resource "github_branch_protection" "charts" {
  repository_id          = github_repository.charts.node_id
  pattern                = "main"
